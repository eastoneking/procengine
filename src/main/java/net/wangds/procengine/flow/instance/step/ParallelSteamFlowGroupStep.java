...
public class ParallelSteamFlowGroupStep<C extends FlowContext> extends AbstractWithCombinFunFlowGroupStep<C> {
    @Override
    protected void executeSubSteps(C ctx, List<FlowStep<C>> subSteps, Map<String, ProcResEnum> subStepResults) {
        subSteps.parallelStream().reduce(subStepResults, (resultMap, curStep)->{
